...
class SLRealizer(object):
    ...
    def deblend(self, lensID=None, null_deblend = False, debug=False):
        ...
        desc.slrealizer.draw_model(self.observation[randomIndex], self.catalog.get_lens(lensID), False, False)
#        plot_lens_random_date(lensID, debug)
        #deblend_distance(lensID)
        return
    ...
    def test_covariance_matrix(self, lensID=None, debug=False, convolve=False):
        ...
        # Now visualize the lens system at the epoch defined by the            
            ...
            print('total_second_moment: ', desc.slrealizer.second_moment(self.observation[randomIndex], self.catalog.get_lens(lensID)))
            print("*******************************")
            desc.slrealizer.covariance_matrix(self.observation[randomIndex], self.catalog.get_lens(lensID))
        return
